...
const CarouselFile = () => {
    ...
    useMemo(() => {
        setActiveProduct((ex) => ({ ...ex, ...AllProductData[0], img: "/checkout_images/product1.jpg" }));
    }, [AllProductData])
    ...
    return (
        <div className='py-4 px-2'>
            ...
            <Carousel autoPlay interval={4000} infiniteLoop showThumbs={false} showStatus={false} showArrows={false} selectedItem={handleActive}>
                ...
                <div>
                    <Image src={"/checkout_images/product1.jpg"} width={1000} height={1000} alt='no' />
                </div>

                <div>
                    <Image src={"/checkout_images/pro_check_2.jpg"} width={1000} height={1000} alt='no' />
                </div>
                <div>
                    <Image src={"/checkout_images/pro_check_3.jpg"} width={1000} height={1000} alt='no' />
                </div>
            ...
            <section className='flex justify-center items-center gap-3 gap-2 md:w-[60%] mx-auto'>
                {
                    ...
                    AllProductData.map((ele, key) => {
                        return <label htmlFor={ele.id} className={`w-full md:w-3/4 mx-auto border text-center rounded-md overflow-hidden my-5 ${ActiveProduct?.id === ele?.id && 'active-data'}`} key={key}>
                            <input type='radio' id={ele.id} name='name' hidden onChange={(e) => {
                                ...
                                setActiveProduct((ex) => ({ ...ex, ...ele, img: key === 0 ? '/checkout_images/product1.jpg' : '/checkout_images/product2.jpg', discount: ActiveProduct?.online ? ele.discount : 0 }))
                            }} />
